@stack($name . '_input_start')
    ...
    <div
        class="form-group col-md-12{{ isset($attributes['required']) ? ' required' : '' }}{{ isset($attributes['readonly']) ? ' readonly' : '' }}{{ isset($attributes['disabled']) ? ' disabled' : '' }}"
        :class="[{'has-error': {{ isset($attributes['v-error']) ? $attributes['v-error'] : 'form.errors.get("' . $name . '")' }} }]">

        <div class="tab-pane tab-example-result fade show active float-left" role="tabpanel" aria-labelledby="-component-tab">
        ...
        @if (!empty($text))
            {!! Form::label($name, $text, ['class' => 'mt-2 ml-2'])!!}
        @endif
