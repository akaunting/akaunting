<div class="flex flex-wrap lg:flex-nowrap items-center justify-between my-16">
    @foreach ($reports as $report)
        <x-link href="{{ route('reports.show', $report->id) }}" class="w-6/12 lg:w-2/12 text-center px-3 group {{ ($loop->count == 6 && $loop->last) ? '' : 'ltr:border-r rtl:border-l border-gray-300' }}"
            override="class"
        >
    ...
    @for ($i = 6; $i > $reports->count(); $i--)
        <div
            @class([
                ...
                'ltr:border-r rtl:border-l border-gray-300' => ($i-1 == $reports->count()) ? false : true,
            ])
        >
