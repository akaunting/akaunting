@if ($paginator->hasPages())
    <ul class="pagination flex items-center justify-end text-black text-sm">
        ...
        @else
            <li class="font-medium ltr:mr-2 rtl:ml-2">
                <a class="page-link flex" href="{{ $paginator->previousPageUrl() }}" rel="prev">
        ...
        @foreach (range(1, $paginator->lastPage()) as $i)
            @if ($i >= $paginator->currentPage() - 2 && $i <= $paginator->currentPage() + 2)
                @if ($i == $paginator->currentPage())
                    <li><button class="page-link font-medium ltr:mr-2 rtl:ml-2 active bg-lilac-300 text-purple px-3 py-1 rounded-lg">{{ $i }}</button></li>
                @else
        ...
        @if ($paginator->hasMorePages())
            <li class="font-medium">
                <a class="page-link flex" href="{{ $paginator->nextPageUrl() }}" rel="next">
        ...
        @else
            <li class="font-medium disabled">
                <button disabled class="page-link flex">
                    <span class="material-icons rtl:rotate-180 px-3 py-1 rounded-lg hover:bg-lilac-300">chevron_right</span>
                </button>
            </li>
        @endif
